#!/usr/bin/env python3
"""FastAPI/MJPEG pothole detection preview for Raspberry Pi 5 + Hailo-8."""
...
logger = logging.getLogger("pothole_web")
app = FastAPI(title="Raspberry Pi 5 Hailo Pothole Detection")
runtime = None
...
class WebRuntime:
    ...
    def stream(self) -> Iterator[bytes]:
        ...
        while not self.stop_event.is_set():
            with self.condition:
                self.condition.wait_for(lambda: self.version != seen or self.stop_event.is_set(), timeout=1)
    ...
    def _loop(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                self.update(frame=frame_id, fps=round(fps_ema, 2), **values)
...
<title>Pothole Detection</title><style>
...
.err{color:#ff7b72;white-space:pre-wrap}.legend{font-size:11px;color:#8b949e;margin-top:10px;line-height:1.6}
@media(max-width:800px){.grid{grid-template-columns:1fr}}
</style></head><body><div class='wrap'><h1>Raspberry Pi 5 · Hailo-8 Pothole Detection</h1><div class='grid'>
<div class='card'><img src='/api/video_feed'></div><div class='card'>
<div class='label'>Inference FPS</div><div class='metric' id='fps'>--</div>
<div class='label'>Visible potholes</div><div class='metric' id='count'>--</div>
<div class='label'>Cumulative events</div><div class='metric' id='cumulative'>--</div>
<div class='label'>Frame</div><div class='metric' id='frame'>--</div>
<div class='state' id='state'>Starting…</div>
<div class='legend'>
<b style='color:#ff5a14'>■</b> Orange = direct Hailo detection (with confidence)<br>
<b style='color:#ffc800'>■</b> Yellow = tracked gap-fill (detection missed this frame)
</div>
</div></div></div><script>
const el=id=>document.getElementById(id);
setInterval(async()=>{try{const s=await (await fetch('/api/status')).json();
el('fps').textContent=s.fps!=null?s.fps.toFixed(1):'--';
el('count').textContent=s.visible_tracks!=null?s.visible_tracks:'--';
el('cumulative').textContent=s.cumulative_events!=null?s.cumulative_events:'--';
el('frame').textContent=s.frame!=null?s.frame:'--';
const st=el('state');
if(s.error){st.textContent='Error: '+s.error;st.className='state err';}
else if(s.running){st.textContent='Running on Hailo-8';st.className='state';}
